// The Asciidoctor "Generate DocBook from AsciiDoc" page documents Asciidoctor's
// `docbook`/`docbook5` backend: how to convert AsciiDoc to DocBook 5 XML and
// feed that XML into a DocBook toolchain (xmlto, fopub, Maven) to produce PDF.
// DocBook is a different output format entirely, produced by a different
// converter; this crate renders HTML5 only and DocBook support is not planned
// (see the workspace README). The page therefore states no rule for this
// renderer to satisfy, so the whole page is tracked as non-normative. See
// `sdd/README.md`.

use crate::tests::sdd::*;

track_file!("ref/asciidoctor/docs/modules/docbook-backend/pages/index.adoc");
