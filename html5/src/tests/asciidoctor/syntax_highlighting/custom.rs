//! Coverage of Asciidoctor's *Custom Syntax Highlighter Adapter* page.
//!
//! This page describes Asciidoctor's Ruby extension API for writing or
//! extending a syntax highlighter adapter (`Asciidoctor::SyntaxHighlighter`).
//! That extension mechanism — and the build-time adapters it customizes
//! (Pygments, Rouge) — is a non-goal for this crate's 1.0: the library exposes
//! no Ruby extension surface and depends only on `asciidoc-parser`. The whole
//! page is tracked as non-normative.

use crate::tests::sdd::*;

track_file!("ref/asciidoctor/docs/modules/syntax-highlighting/pages/custom.adoc");

// Registering a custom Ruby syntax-highlighter adapter (and extending the
// build-time built-ins) is a non-goal for this crate's 1.0 — there is no Ruby
// extension surface here — so none of this page's claims are verifiable. The
// entire page is reproduced for coverage as non-normative.
non_normative!(
    r#"
