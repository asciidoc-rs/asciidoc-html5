//! Coverage of Asciidoctor's *Rouge* page.
//!
//! Rouge is a *build-time* (server-side) syntax highlighter: it tokenizes
//! source into `<span>` markup during conversion by invoking an external Ruby
//! gem. Server-side syntax highlighting is a settled non-goal for this crate's
//! 1.0 — the library depends only on `asciidoc-parser`, and no in-process
//! highlighter can match the Asciidoctor 2.0.26 parity oracle byte for byte —
//! so the whole page is tracked as non-normative.

use crate::tests::sdd::*;

track_file!("ref/asciidoctor/docs/modules/syntax-highlighting/pages/rouge.adoc");

// Rouge is a build-time (server-side) syntax highlighter. Supporting it —
// installing/invoking the rouge gem and emitting its `<span>` token markup — is
// a non-goal for this crate's 1.0, so none of this page's claims are verifiable
// here. The entire page is reproduced for coverage as non-normative.
non_normative!(
    r#"
= Rouge
