//! Coverage of Asciidoctor's *Pygments* page.
//!
//! Pygments is a *build-time* (server-side) syntax highlighter: it tokenizes
//! source into `<span>` markup during conversion by shelling out to an external
//! Python program via the pygments.rb gem. Server-side syntax highlighting is a
//! settled non-goal for this crate's 1.0 — the library depends only on
//! `asciidoc-parser`, and no in-process highlighter can match the Asciidoctor
//! 2.0.26 parity oracle byte for byte — so the whole page is tracked as
//! non-normative.
...
// Pygments is a build-time (server-side) syntax highlighter. Supporting it —
// installing/invoking Python and the pygments.rb gem and emitting its `<span>`
// token markup — is a non-goal for this crate's 1.0, so none of this page's
