...
// Asciidoctor's "Localization Support" page. Most of it describes features this
// renderer does not implement -- DocBook-toolchain label translation, the full
// table of built-in caption/label attributes (whose blocks this crate does not
// render yet), the `lang`-driven `locale/attributes.adoc` include, and content
// translation -- so those spans are tracked as non-normative. Three claims have
// counterparts this crate can verify and does: UTF-8 content passes through
// unchanged, the `lang` attribute is only a hint (it does not translate
// built-in labels for HTML), and the one built-in label this renderer emits
// today -- `version-label`, the revision label in the header -- honors its
// attribute. The `adoc` crate verifies the same three claims through the CLI,
...
"#
    ...
    let source = "Café, naïve, Über. 日本語. Ελληνικά. مرحبا.";
...
// The `lang` attribute is only a hint: it sets the html `lang` but does not
// translate any built-in label for HTML output.
#[test]
...
"#
    ...
    // With `:lang: es`, the html element records the language, but the built-in
    ...
    let source = "= Título\nAutora\nv1.0, 2026-01-04\n:lang: es\n";
    let html = convert_with(source, &Options::new().standalone(true));
...
// `version-label` is the one built-in label this renderer emits today: it names
// the revision number in the header (Asciidoctor downcases it). Overriding it
// with a translated value changes the rendered label.
#[test]
...
"#
    ...
    // Override `version-label` with the German "Ausgabe"; the header renders it
    // downcased, matching Asciidoctor.
    let translated = convert_with(
        "= Titel\nAutor\nv2.5, 2026-01-04\n:version-label: Ausgabe\n",
