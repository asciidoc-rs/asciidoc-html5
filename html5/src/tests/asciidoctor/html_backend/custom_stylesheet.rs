...
// crates) rather than duplicated here. The `copycss` file copy is likewise
// supported now and verified on that same docs page, so it stays non-normative
// here. Fetching a remote (`http`) stylesheet to embed it is *not planned* —
// neither the library nor the CLI reads over the network — so that path stays
// non-normative too.
//
