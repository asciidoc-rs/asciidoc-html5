...
// Asciidoctor's own introduction page. It is an overview of the Ruby
// Asciidoctor project — its history, packaging, ecosystem, and the JVM and
// JavaScript platforms it runs on — none of which describe a rendering rule
// this crate can verify, so nearly the whole page is non-normative here. The
// one testable thing it shows is the "Basic usage" invocation. This crate
// verifies the API column against its closest available API, `convert_file`
// (see `basic_usage_via_convert_file`); the `adoc` crate verifies the CLI
// column, and the sdd tool merges the two.
...
// The API column of the "Basic usage" table. Asciidoctor's `convert_file`
// reads an AsciiDoc file from disk and renders it in one call; this crate's
// closest available API is `asciidoc_html5::convert_file`, which this test
// exercises. (The `safe: :safe` argument is an Asciidoctor sandboxing mode
// with no counterpart in this crate, which performs no file includes.)
#[test]
fn basic_usage_via_convert_file() {
    verifies!(
        r#"
a|
[,ruby]
