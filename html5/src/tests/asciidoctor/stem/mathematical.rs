//! Coverage of Asciidoctor's *Asciidoctor Mathematical* page.
//!
//! Asciidoctor Mathematical is a Ruby extension that visits each STEM node and
//! rasterizes the expression to an image during conversion, replacing the STEM
//! node with an image node. It is backed by the `mathematical` gem, a *native*
//! library. Taking that native dependency is a settled non-goal for this crate
//! — the library depends only on `asciidoc-parser` — so the whole page is
//! tracked as non-normative.

use crate::tests::sdd::*;

track_file!("ref/asciidoctor/docs/modules/stem/pages/mathematical.adoc");

// Asciidoctor Mathematical is a Ruby extension backed by the native
// `mathematical` gem, which rasterizes each STEM expression to an image during
// conversion. Requiring that native dependency is a non-goal for this crate's
// 1.0 (the library depends only on `asciidoc-parser`), so none of this page's
// claims are verifiable here. The entire page is reproduced for coverage as
// non-normative.
non_normative!(
    r#"
