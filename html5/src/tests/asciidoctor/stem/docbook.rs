//! Coverage of Asciidoctor's *STEM Support in the DocBook Toolchain* page.
//!
//! This page describes STEM processing in the DocBook toolchain (the dblatex
//! and fop PDF pipelines). That is only relevant to the DocBook backend, which
//! this crate does not target, so the whole page is tracked as non-normative.

use crate::tests::sdd::*;

track_file!("ref/asciidoctor/docs/modules/stem/pages/docbook.adoc");

// STEM support in the DocBook toolchain (dblatex/fop pipelines) is only
// relevant to the DocBook backend, which this crate does not target, so none
// of this page's claims are verifiable here. The entire page is reproduced for
// coverage as non-normative.
non_normative!(
    r#"
