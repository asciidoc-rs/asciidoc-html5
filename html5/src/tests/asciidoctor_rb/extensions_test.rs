//! Tracks Asciidoctor's `extensions_test.rb`.
//!
//! This file tests Asciidoctor's **extension mechanism** — the registry and
//! the preprocessor / tree-processor / postprocessor / block / block-macro /
//! inline-macro / include-processor extension points. That mechanism is
//! explicitly **out of scope** for this project: the README states "An
//! extension mechanism is not planned for 1.0." This renderer implements none
//! of those extension points, so there is no behavior here to verify.
//!
//! Like `manpage_test.rb`, this is out of scope by *project decision*, not
//! because it tests a lower layer (see the crate memory
//! `implement-missing-over-non-normative`, whose one standing exception is
//! README-declared out-of-scope items). Tracked entirely as `non_normative!` so
//! the page is honestly accounted for rather than counted as uncovered.
//!
//! The file is reproduced with the `r##"…"##` raw-string delimiter because its
//! content contains the `"#` sequence, which would close the usual `r#"…"#`.
