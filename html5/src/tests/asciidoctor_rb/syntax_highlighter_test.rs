...
//! For those, the converter passes the source through unchanged and adds the
//! language metadata and CDN assets, so the Highlight.js and Prettify tests
//! port directly, driven through `convert`/`convert_with`. The generic
//! language-tagging tests (a source block is tagged with its language whether
//! or not a recognized highlighter is set) and the safe-mode lock (a document
//! may not enable a highlighter at SERVER or above) port too.
//!
//! Kept `non_normative!` is everything this crate's stack does not produce: the
//! *build-time* highlighters CodeRay, Rouge, and Pygments (a settled non-goal —
//! the library depends only on `asciidoc-parser`, and no in-process highlighter
//! can match the Asciidoctor 2.0.26 parity oracle byte for byte); the Ruby
//! `SyntaxHighlighter` registration/extension API and the
//! `doc.syntax_highlighter` property; the DocBook backend; and the
//! `html-pipeline` highlighter (which this crate does not implement — a
//! `[source]` block falls back to the default language-tagged shape rather than
//! Asciidoctor's `pre[lang]` output).
...
// The `doc.syntax_highlighter` document property (whether it is set, and to
// which adapter) is part of Asciidoctor's Ruby document model; this crate
// exposes no such property, and one case targets the DocBook backend. Neither
// is observable through `convert`, so these are tracked but not verified.
non_normative!(
    r#"
...
#[test]
fn should_not_allow_document_to_enable_syntax_highlighter_if_safe_mode_is_at_least_server() {
    verifies!(
        r#"
...
"#
    ...
    // The reproduced test uses coderay, a build-time highlighter this crate
    // does not implement (a settled non-goal). The claim under test is the
    // safe-mode lock itself: a document may not enable a syntax highlighter at
    // SERVER or above. We observe it with highlight.js, the client-side
    // highlighter this crate does implement — under SERVER a document-set
    // `:source-highlighter:` is dropped, so no highlighter markup or CDN asset
    // is emitted and the source block falls back to its default shape.
    let html = convert_with(
        "= Doc\n:source-highlighter: highlight.js\n\n[source,ruby]\n----\nputs 'Hello, World!'\n----\n",
        &Options::new().safe_mode(SafeMode::Server),
    );
    assert_css(&html, "pre.highlightjs.highlight", 0);
    assert!(!html.contains("hljs"), "{html}");
    assert!(!html.contains("cdnjs.cloudflare.com"), "{html}");
}

// The `SyntaxHighlighter` registration and extension API (`register_for`, a
// custom adapter subclass, the `highlight?`/`format` hooks) is a Ruby
// extension surface this crate does not expose, so these cannot be verified.
non_normative!(
    r##"
...
#[test]
fn should_set_language_on_output_of_source_block_when_source_highlighter_attribute_is_not_set() {
    verifies!(
        r#"
...
"#
    ...
    assert_css(&html, "pre.highlight", 1);
    assert_css(&html, "pre.highlight > code.language-ruby", 1);
    assert_css(
        &html,
...
#[test]
fn should_set_language_on_output_of_source_block_when_source_highlighter_attribute_is_not_recognized(
) {
    verifies!(
        r#"
...
// These exercise build-time CodeRay markup, the DocBook backend, a
// `source-language` promoted into CodeRay output, and the compat-mode
// `language`->`source-language` attribute rename (a parser-level concern).
// None is observable through this crate's html5 `convert`.
non_normative!(
    r#"
...
"#
    ...
    #[test]
    fn should_add_data_lang_as_last_attribute_on_code_tag_when_source_highlighter_is_highlight_js()
    {
        verifies!(
            r#"
...
"#
        ...
        assert!(
            html.contains("<code class=\"language-ruby hljs\" data-lang=\"ruby\">"),
            "{html}"
        );
    }

    #[test]
    fn should_include_remote_highlight_js_assets_if_source_highlighter_attribute_is_highlight_js() {
        verifies!(
            r#"
...
"#
        ...
        assert_css(&html, "pre.highlightjs.highlight", 1);
        assert_css(
            &html,
            "pre.highlightjs.highlight > code.language-html.hljs[data-lang=\"html\"]",
            1,
        );
        assert!(html.contains("&lt;p&gt;Highlight me!&lt;/p&gt;"), "{html}");
        assert_css(&html, "head > link[href*=\"highlight.js\"]", 1);
        assert_css(&html, "#footer ~ script[src*=\"highlight.min.js\"]", 1);
        assert_xpath(
            &html,
            "//script[contains(text(), \"hljs.highlightBlock(el)\")]",
            1,
        );
    }

    #[test]
    fn should_add_language_none_class_to_source_block_when_source_highlighter_is_highlight_js_and_language_is_not_set(
    ) {
        verifies!(
            r#"
...
"#
        ...
        assert_css(&html, "code.language-none", 1);
    }

    #[test]
    fn should_load_additional_languages_specified_by_highlightjs_languages() {
        verifies!(
            r#"
...
"#
        ...
        assert_css(&html, "#footer ~ script[src*=\"languages/yaml.min.js\"]", 1);
        assert_css(
            &html,
            "#footer ~ script[src*=\"languages/scilab.min.js\"]",
            1,
        );
    }
...
"#
    ...
    #[test]
    fn should_add_language_classes_to_child_code_element_when_source_highlighter_is_prettify() {
        verifies!(
            r#"
...
"#
        ...
        // Asciidoctor sets `source-highlighter` here via the API `attributes`
        // option; the crate's counterpart is `Options::attribute`, honored in
        // every safe mode (an API value is trusted, unlike a document-set one).
        let html = convert_with(
            "[source,ruby]\n----\nputs \"foo\"\n----\n",
            &Options::new().attribute("source-highlighter", "prettify"),
        );
        assert_css(&html, "pre[class=\"prettyprint highlight\"]", 1);
        assert_css(&html, "pre > code[data-lang=\"ruby\"]", 1);
    }

    #[test]
    fn should_set_linenums_start_if_linenums_are_enabled_and_start_attribute_is_set_when_source_highlighter_is_prettify(
    ) {
        verifies!(
            r#"
...
"#
        ...
        assert_css(&html, "pre[class=\"prettyprint highlight linenums:5\"]", 1);
        assert_css(&html, "pre > code[data-lang=\"ruby\"]", 1);
    }
...
"#
        ...
        assert!(html.contains(" # <b class=\"conum\">(1)</b>"), "{html}");
        assert!(html.contains(" # <b class=\"conum\">(2)</b>"), "{html}");
    }
...
"#
        ...
        let html = convert("----\n<parent> <!--1-->\n  <child/> <!--2-->\n</parent>\n----\n");
        assert!(
            html.contains(" &lt;!--<b class=\"conum\">(1)</b>--&gt;"),
            "{html}"
        );
        assert!(
            html.contains(" &lt;!--<b class=\"conum\">(2)</b>--&gt;"),
            "{html}"
        );
    }
