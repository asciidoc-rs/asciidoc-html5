...
//! parser/core utility, not a rendering concern.
//!
//! This crate is an HTML5 renderer: it neither exposes nor reimplements
//! `PathResolver`, and none of these claims has a rendered form to verify (see
//! the "test rendered output, not internals" rule in this directory's README).
//! The one place the renderer touches web-path resolution is
//! [`crate::renderer::normalize_web_path`], a minimal, stylesheet-specific port
//! of Asciidoctor's `normalize_web_path(stylesheet, stylesdir)` helper used
//! only to compute a custom stylesheet's `href`; its behavior is covered
//! directly by unit tests in `renderer.rs`, not through this general-purpose
//! suite.
