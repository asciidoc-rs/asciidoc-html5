//! Tracks Asciidoctor's `converter_test.rb`.
//!
//! This file tests Asciidoctor's **pluggable converter architecture**, not the
//! HTML5 rendering this crate reproduces. Its three contexts all exercise
//! Ruby-only plumbing:
//!
//! - *View options* / *Custom backends* — the `TemplateConverter`, which
//!   renders nodes from external Haml / Slim / ERB template files, plus its
//!   template cache and `template_dir` / `template_engine_options`
//!   configuration. This renderer has a single built-in converter and no
//!   template engine.
//! - *Custom converters* — the converter registry and factories
//!   (`register_for`, `Converter.for`, `CustomFactory`, `DefaultFactoryProxy`,
//!   `derive_backend_traits`, catch-all `*` converters, `supports_templates?`).
//!   That is the plugin API by which third parties swap in their own converter.
//!
//! This is the same pluggable-backend/extension machinery the README places out
//! of scope: "An extension mechanism is not planned for 1.0." Like
//! `extensions_test.rb`, it is out of scope by *project decision*, not because
//! it tests a lower layer (see the crate memory
//! `implement-missing-over-non-normative`, whose one standing exception is
//! README-declared out-of-scope items).
//!
//! The handful of tests that touch the built-in HTML5 converter's observable
//! output are themselves out of scope or covered elsewhere:
...
//!   - the `convert_`-prefix delegation tests exercise Ruby `method_missing`.
//!
//! Tracked entirely as `non_normative!` so the file is honestly accounted for
