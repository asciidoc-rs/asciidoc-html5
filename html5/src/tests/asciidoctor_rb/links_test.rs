...
//!
//! Inline links, autolinks, cross references (`xref` / `<<>>`), and inline
//! anchors are all produced by `asciidoc-parser`'s default inline
//! substitution renderer, which this crate uses unchanged (it renders blocks,
//! not inline markup). The overwhelming majority of these tests therefore port
//! directly, driven through `convert` (embedded, the counterpart to
...
//! crate will not implement compat mode (#124, closed won't-do); other inline
//! behavior `asciidoc-parser` diverges on (an inter-document `xref:` whose path
...
/// `assert_include` / `assert_includes` / `assert_match` on a literal string).
#[track_caller]
...
"###
    ...
    assert_xpath(
        &html,
        r####"//a[@href='http://asciidoc.org'][@class='bare'][text() = 'http://asciidoc.org']"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &html,
        r####"//a[@href='http://asciidoc.org'][@class='bare project'][text() = 'http://asciidoc.org']"####,
        1,
    );
}
...
"###
    ...
    let html = convert("file:///home/user/bookmarks.html[My Bookmarks]");
    assert_xpath(
        &html,
        r####"//a[@href='file:///home/user/bookmarks.html'][text() = 'My Bookmarks']"####,
        1,
    );
}
...
"###
    ...
    // The point of this test is that the scheme is *not* hidden; this crate
    // never hides it, so the implicit link text keeps the scheme regardless
    // of the `hide-uri-scheme` attribute.
    for (input, expected) in [("link:https://[]", "https://"), ("link:ssh://[]", "ssh://")] {
        let html = convert_with(
            input,
            &Options::new()
                .doctype("inline")
        ...
        assert_xpath(&html, &format!(r####"/a[text() = "{expected}"]"####), 1);
    }
}
...
"###
    ...
    assert_xpath(
        &html,
        r####"//a[@href='http://asciidoc.org'][text() = 'AsciiDoc']"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &html,
        r####"//a[@href='http://asciidoc.org'][text() = '[Ascii]Doc']"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &html,
        r"//a[@href='https://google.com'][text() = 'Google for \']",
        1,
    );
}
...
"###
    ...
    // Only the HTML (inline doctype) branch is reproducible here; the
    // DocBook assertion targets a backend this crate does not implement.
    let html = convert_with(
...
"###
    ...
    let output = convert("Link to link:[this page].");
    assert_xpath(&output, "//a", 1);
    assert_xpath(&output, r####"//a[@href=""]"####, 1);
}
...
"###
    ...
    let output = convert("<<https://asciidoc.org>>");
    assert_includes(
        &output,
...
"###
    ...
    let html = convert_with("\\<http://asciidoc.org>", &Options::new().standalone(true));
    assert_xpath(&html, r####"//p[text()="<http://asciidoc.org>"]"####, 1);
}
...
"###
    ...
    let output = convert("<<https://example.com, Example>>");
    assert_includes(
        &output,
...
"###
    ...
    let output = convert("<<link:https://example.com[], Example>>");
    assert_includes(
        &output,
...
"###
    ...
    let output = convert("https://github.com/<org>/");
    assert_includes(
        &output,
...
"###
    ...
    assert_xpath(
        &html,
        r####"//a[@href="http://asciidoc.org"][text()="http://asciidoc.org"]"####,
        1,
    );
}
...
"###
    ...
    let result = convert("The homepage for Asciidoctor is https://asciidoctor.org.");
    assert_xpath(
        &result,
        r####"//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]"####,
        1,
    );
    assert_xpath(
        &result,
        r####"//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]/following-sibling::text()[starts-with(., ".")]"####,
        1,
    );
}
...
"###
    ...
    let result = convert("Check out https://asciidoctor.org!");
    assert_xpath(
        &result,
        r####"//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]"####,
        1,
...
"###
    ...
    let result = convert("Is the homepage for Asciidoctor https://asciidoctor.org?");
    assert_xpath(
        &result,
        r####"//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]"####,
        1,
...
"###
    ...
    let result =
        convert("Asciidoctor is a Ruby-based AsciiDoc processor (see https://asciidoctor.org)");
    assert_xpath(
        &result,
        r####"//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]"####,
        1,
...
"###
    ...
    let result = convert("https://asciidoctor.org; where text gets parsed");
    assert_xpath(
        &result,
        r####"//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]"####,
        1,
...
"###
    ...
    let result = convert("(from https://asciidoctor.org): where text gets parsed");
    assert_xpath(
        &result,
        r####"//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]"####,
        1,
...
"###
    ...
    // The Ruby test asserts on the parsed block's substituted `content`; the
    // rendered paragraph carries the same text and creates no link.
    for input in ["http://;", "file://:", "irc://,"] {
        let html = convert(input);
        assert_xpath(&html, "//a", 0);
        assert_xpath(&html, &format!(r####"//p[text()="{input}"]"####), 1);
    }
}
...
"###
    ...
    for (input, text) in [("(https://)", "(https://)"), ("<ftp://>", "<ftp://>")] {
        let html = convert(input);
        assert_xpath(&html, "//a", 0);
        assert_xpath(&html, &format!(r####"//p[text()="{text}"]"####), 1);
    }
}
...
"###
    ...
    let html = convert_with("http://jruby.org/apidocs/org/jruby/Ruby.html#addModule(org.jruby.RubyModule)[addModule() adds a Ruby module]", &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="http://jruby.org/apidocs/org/jruby/Ruby.html#addModule(org.jruby.RubyModule)"][text()="addModule() adds a Ruby module"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert_with(
        "\u{a0}http://asciidoc.org[AsciiDoc] project page.",
        &Options::new().standalone(true),
    );
    assert_xpath(
        &html,
...
"###
    ...
    let output = convert("l&#8217;http://www.irit.fr[IRIT]");
    assert_includes(&output, "l&#8217;<a href=");
}
...
"###
    ...
    let output = convert(r####""https://asciidoctor.org[]""####);
    assert_includes(
        &output,
...
"###
    ...
    let output = convert("'https://asciidoctor.org[]'");
    assert_includes(
        &output,
        r####"'<a href="https://asciidoctor.org" class="bare">https://asciidoctor.org</a>'"####,
    );
...
"###
    ...
    let result = convert("Information about the https://symbols.example.org/.[.] character.");
    assert_xpath(
        &result,
        r####"//a[@href="https://symbols.example.org/."][text()="."]"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(&output, "//a", 0);
    assert_xpath(
        &output,
        r####"//p[starts-with(text(), "http://asciidoc.org[asciidoc.org]")]"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &html,
        r####"//a[@href="http://xircles.codehaus.org/lists/dev@geb.codehaus.org"][text()="subscribe"]"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &html,
        r####"//a[@href="http://xircles.codehaus.org/lists/dev@geb.codehaus.org"][text()="http://xircles.codehaus.org/lists/dev@geb.codehaus.org"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert_with("The source code for Asciidoctor can be found at https://github.com/asciidoctor\nwhich is a GitHub organization.", &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="https://github.com/asciidoctor"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert_with("The source code for Asciidoctor can be found at link:https://github.com/asciidoctor\n[]which is a GitHub organization.", &Options::new().standalone(true));
    assert_xpath(&html, "//a", 0);
}
...
"###
    ...
    let html = convert_with("I often need to refer to the chapter on link:http://asciidoc.org?q=attribute references[Attribute References].", &Options::new().standalone(true));
    assert_xpath(&html, "//a", 0);
}
...
"###
    ...
    let html = convert_with("I often need to refer to the chapter on link:http://asciidoc.org?q=attribute%20references[Attribute References].", &Options::new().standalone(true));
...
"###
    ...
    let output =
        convert(r####"http://search.example.com["Google, Yahoo, Bing = Search Engines"]"####);
    assert_xpath(
        &output,
        r####"//a[@href="http://search.example.com"][text()="Google, Yahoo, Bing = Search Engines"]"####,
        1,
    );
}
...
"###
    ...
    let output = convert("https://example.com[What You Need\n= What You Get]");
    assert_xpath(
        &output,
...
"###
    ...
    let output = convert(r####"http://search.example.com["Google, Yahoo, Bing"]"####);
    assert_xpath(
        &output,
        r####"//a[@href="http://search.example.com"][text()='"Google, Yahoo, Bing"']"####,
        1,
    );
}
...
"###
    ...
    let output = convert("http://search.example.com[Google, Yahoo, Bing]");
    assert_xpath(
        &output,
        r####"//a[@href="http://search.example.com"][text()="Google, Yahoo, Bing"]"####,
        1,
...
"###
    ...
    let result = convert("https://example.com[[.role]#Foo\nBar#]");
    assert_includes(
        &result,
...
"###
    ...
    let html = convert(r####"http://google.com[Google, role=external, window="_blank"]"####);
    assert_xpath(
        &html,
        r####"//a[@href="http://google.com"][@class="external"][@target="_blank"]"####,
        1,
    );
}
...
"###
    ...
    let result = convert("https://example.com[Foo\nBar,role=foobar]");
    assert_includes(
        &result,
...
"###
    ...
    let url = "https://fonts.googleapis.com/css?family=Roboto:400,400italic,";
    let html = convert(&format!("link:{url}[family=Roboto,weight=400]"));
    assert_xpath(
        &html,
        &format!(r####"//a[@href="{url}"][text()="{url}"]"####),
        1,
    );
}
...
"###
    ...
    let url = "https://fonts.googleapis.com/css?family=Roboto:400,400italic,";
    let html = convert(&format!("link:{url}[Roboto,400]"));
    assert_xpath(
        &html,
        &format!(r####"//a[@href="{url}"][text()="Roboto,400"]"####),
        1,
    );
}
...
"###
    ...
    let url = "https://fonts.googleapis.com/css?family=Roboto:400";
    let html = convert(&format!("link:{url}[,id=roboto-regular,role=font]"));
    assert_xpath(
        &html,
        &format!(
            r####"//a[@href="{url}"][@id="roboto-regular"][@class="bare font"][text()="{url}"]"####
        ),
        1,
    );
}
...
"###
    ...
    let html = convert("http://google.com[Google^]");
    assert_xpath(
        &html,
        r####"//a[@href="http://google.com"][@target="_blank"]"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &html,
        r####"//a[@href="http://google.com"][@target="_blank"][@rel="noopener"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert("http://google.com[Google,window=name,opts=noopener]");
    assert_xpath(
        &html,
        r####"//a[@href="http://google.com"][@target="name"][@rel="noopener"]"####,
        1,
    );
}
...
"###
    ...
    let result = convert("http://google.com[Google,opts=noopener]");
    assert_xpath(&result, r####"//a[@href="http://google.com"]"####, 1);
    assert_xpath(
        &result,
        r####"//a[@href="http://google.com"][@rel="noopener"]"####,
        0,
    );
}
...
"###
    ...
    let html = convert(r####"http://google.com[Google,window=name,opts="nofollow,noopener"]"####);
    assert_xpath(
        &html,
        r####"//a[@href="http://google.com"][@target="name"][@rel="nofollow noopener"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert(r####"http://google.com[Google, id="link-1"]"####);
    assert_xpath(
        &html,
        r####"//a[@href="http://google.com"][@id="link-1"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert(r####"http://google.com[Google, title="title-1"]"####);
    assert_xpath(
        &html,
        r####"//a[@href="http://google.com"][@title="title-1"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert("irc://irc.freenode.net");
    assert_xpath(
        &html,
        r####"//a[@href="irc://irc.freenode.net"][text()="irc://irc.freenode.net"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert("irc://irc.freenode.net[Freenode IRC]");
    assert_xpath(
        &html,
        r####"//a[@href="irc://irc.freenode.net"][text()="Freenode IRC"]"####,
        1,
    );
}
...
"###
    ...
    let output = convert("[[:idname]] text");
    assert_xpath(&output, r####"//a[@id=":idname"]"####, 1);
}
...
"###
    ...
    let output = convert("[[1-install]] text");
    assert_includes(&output, "[[1-install]]");
    assert_xpath(&output, r####"//a[@id = "1-install"]"####, 0);
}
...
"###
    ...
    assert_includes(&result, r####"see <a href="#foo">[FOO]</a>"####);
}
...
"###
    ...
    let input = "see <<foo>>\n\nanchor:foo[b[a\\]r]tex\n";
    let result = convert(input);
    assert_includes(&result, r####"see <a href="#foo">b[a]r</a>"####);
}
...
#[test]
fn xref_using_angled_bracket_syntax() {
    verifies!(
        r###"
...
"###
    ...
    // The Ruby test registers the ref manually; an undefined `<<tigers>>`
    // renders the identical broken-reference output ([tigers] → #tigers).
    let html = convert_with("<<tigers>>", &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="#tigers"][text() = "[tigers]"]"####,
        1,
    );
}

#[test]
fn xref_using_angled_bracket_syntax_with_explicit_hash() {
    verifies!(
        r###"
...
"###
    ...
    // A real section supplies the reftext the Ruby test registers by hand.
    let html = convert_with(
        "<<#tigers>>\n\n[#tigers]\n== Tigers\n",
        &Options::new().standalone(true),
    );
    assert_xpath(&html, r####"//a[@href="#tigers"][text() = "Tigers"]"####, 1);
}
...
"###
    ...
    let input = "<<tigers,About Tigers>>\n\n[#tigers]\n== Tigers\n";
    let html = convert_with(input, &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="#tigers"][text() = "About Tigers"]"####,
        1,
    );
}
...
// will not implement compat mode (#124, closed won't-do); the parser renders
// the inter-document form instead of the plain internal reference.
non_normative!(
    r###"
...
"###
    ...
    let html = convert("<<tigers#>>");
    assert_xpath(
        &html,
        r####"//a[@href="tigers.html"][text() = "tigers.html"]"####,
        1,
    );
}
...
"###
    ...
    for (target, text) in [
        ...
        let result = convert(&format!("<<{target},{text}>>"));
        let stem = target.strip_suffix('#').unwrap();
        assert_xpath(
            &result,
            &format!(r####"//a[@href="{stem}.html"][text()="{text}"]"####),
            1,
        );
    }
}
...
"###
    ...
    for (target, text) in [
        ...
        let result = convert(&format!("xref:{target}[{text}]"));
        let stem = target.strip_suffix('#').unwrap();
        assert_xpath(
            &result,
            &format!(r####"//a[@href="{stem}"][text()="{text}"]"####),
            1,
        );
    }
    for (target, text) in [
...
"###
    ...
    for (path, text) in [
        ...
        let result = convert(&format!("xref:{path}[{text}]"));
        assert_xpath(
            &result,
            &format!(r####"//a[@href="{path}"][text()="{text}"]"####),
            1,
        );
    }
    let result = convert("xref:sections.d/first[First Section]");
    assert_xpath(
        &result,
        r####"//a[@href="#sections.d/first"][text()="First Section"]"####,
        1,
    );
}
...
"###
    ...
    let result = convert("<<using-.net-web-services.adoc#,Using .NET web services>>");
    assert_xpath(
        &result,
        r####"//a[@href="using-.net-web-services.html"][text() = "Using .NET web services"]"####,
        1,
    );
}

#[test]
fn xref_macro_target_containing_dot_should_be_interpreted_as_a_path_unless_prefixed_by() {
    verifies!(
        r###"
...
"###
    ...
    let result = convert("xref:using-.net-web-services[Using .NET web services]");
    assert_xpath(
        &result,
        r####"//a[@href="using-.net-web-services"][text() = "Using .NET web services"]"####,
        1,
    );
    let result = convert("xref:#using-.net-web-services[Using .NET web services]");
    assert_xpath(
...
"###
    ...
    let result = convert("xref:doc\\__with_double__underscore.adoc[text]");
    assert_xpath(
        &result,
        r####"//a[@href="doc__with_double__underscore.html"][text() = "text"]"####,
        1,
    );
}
...
"###
    ...
    let result = convert("<<doc\\__with_double__underscore.adoc#,text>>");
    assert_xpath(
        &result,
        r####"//a[@href="doc__with_double__underscore.html"][text() = "text"]"####,
        1,
...
"###
    ...
    let html = convert("<<../tigers#,tigers>>");
    assert_xpath(
        &html,
        r####"//a[@href="../tigers.html"][text() = "tigers"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert("<</path/to/tigers#,tigers>>");
    assert_xpath(
        &html,
        r####"//a[@href="/path/to/tigers.html"][text() = "tigers"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert("<<tigers.adoc>>");
    assert_xpath(
        &html,
        r####"//a[@href="#tigers.adoc"][text() = "[tigers.adoc]"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert("<<tigers.adoc#id>>");
    assert_xpath(
        &html,
        r####"//a[@href="tigers.html#id"][text() = "tigers.html"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert("xref:tigers.adoc[]");
    assert_xpath(
        &html,
        r####"//a[@href="tigers.html"][text() = "tigers.html"]"####,
        1,
...
"###
    ...
    let html = convert("<<tigers#about>>");
    assert_xpath(
        &html,
        r####"//a[@href="tigers.html#about"][text() = "tigers.html"]"####,
        1,
    );
}
...
"###
    ...
    let html = convert("<<tigers#about,About Tigers>>");
    assert_xpath(
        &html,
        r####"//a[@href="tigers.html#about"][text() = "About Tigers"]"####,
        1,
    );
}
...
"###
    ...
    let input = "Want to learn <<tigers,about tigers>>?\n\n[#tigers]\n== Tigers\n";
    let html = convert_with(input, &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="#tigers"][text() = "about tigers"]"####,
...
"###
    ...
    let input = "Want to learn <<tigers,about\ntigers>>?\n\n[#tigers]\n== Tigers\n";
    let html = convert_with(input, &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="#tigers"][normalize-space(text()) = "about tigers"]"####,
        1,
    );
}
...
"###
    ...
    let input = "See the <<tigers, `+[tigers]+`>> section for details about tigers.\n\n[#tigers]\n== Tigers\n";
    let output = convert(input);
    assert_xpath(
        &output,
        r####"//a[@href="#tigers"]/code[text()="[tigers]"]"####,
        1,
    );
}
...
"###
    ...
    for xref in [
        ...
        let input = format!(":lessonsdir: lessons\n\n[#lesson-1-listing]\n== {xref}\n\nA summary of the first lesson.\n");
        let output = convert(&input);
        assert_xpath(&output, r####"//h2/a[@href="lessons/lesson-1.html"]"####, 1);
    }
}

#[test]
fn xref_using_macro_syntax() {
    verifies!(
        r###"
...
"###
    ...
    // Undefined xref renders identically to the Ruby test's registered ref.
    let html = convert_with("xref:tigers[]", &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="#tigers"][text() = "[tigers]"]"####,
        1,
...
"###
    ...
    let input = "This document has two sections, xref:sect-a[] and xref:sect-b[].\n\n[#sect-a]\n== Section A\n\n[#sect-b]\n== Section B\n";
    let result = convert(input);
    assert_xpath(
        &result,
        r####"//a[@href="#sect-a"][text() = "Section A"]"####,
        1,
    );
    assert_xpath(
        &result,
        r####"//a[@href="#sect-b"][text() = "Section B"]"####,
        1,
    );
}

#[test]
fn xref_using_macro_syntax_with_explicit_hash() {
    verifies!(
        r###"
...
"###
    ...
    let input = "xref:tigers[About Tigers]\n\n[#tigers]\n== Tigers\n";
    let html = convert_with(input, &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="#tigers"][text() = "About Tigers"]"####,
...
"###
    ...
    let input = "Want to learn xref:tigers[about tigers]?\n\n[#tigers]\n== Tigers\n";
    let html = convert_with(input, &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="#tigers"][text() = "about tigers"]"####,
...
"###
    ...
    let input = "Want to learn xref:tigers[about\ntigers]?\n\n[#tigers]\n== Tigers\n";
    let html = convert_with(input, &Options::new().standalone(true));
    assert_xpath(
        &html,
        r####"//a[@href="#tigers"][normalize-space(text()) = "about tigers"]"####,
...
"###
    ...
    let input = "xref:tigers[[tigers\\] are cats]\n\n[#tigers]\n== Tigers\n";
    let output = convert(input);
    assert_xpath(
        &output,
        r####"//a[@href="#tigers"][text() = "[tigers] are cats"]"####,
        1,
    );
}
...
"###
    ...
    let input = "anchor:foo[b[a\\]r]about\n\nsee <<foo>>\n";
    let result = convert(input);
    assert_xpath(&result, r####"//a[@href="#foo"]"####, 1);
    assert_xpath(&result, r####"//a[@href="#foo"][text()="b[a]r"]"####, 1);
}

#[test]
fn xref_using_invalid_macro_syntax_does_not_create_link() {
    verifies!(
        r###"
...
"###
    ...
    // `xref:tigers` (no target brackets) is not a valid macro, so no link.
    let html = convert_with("xref:tigers", &Options::new().standalone(true));
    assert_xpath(&html, "//a", 0);
}
...
#[test]
fn should_not_warn_if_verbose_flag_is_set_and_reference_is_found_in_compat_mode() {
    verifies!(
        r###"
...
#[test]
fn should_warn_and_create_link_if_verbose_flag_is_set_and_reference_using_notation_is_not_found() {
    verifies!(
        r###"
...
"###
    ...
    assert_xpath(
        &output,
        r####"//h2[@id="_section_a"][text()="Section A"]"####,
        1,
    );
    assert_xpath(
        &output,
        r####"//a[@href="#_section_a"][text()="Section A"]"####,
        1,
    );
    assert_xpath(
        &output,
        r####"//h2[@id="_section_b"][text()="Section B"]"####,
        1,
    );
    assert_xpath(
        &output,
        r####"//a[@href="#_section_b"][text()="Section B"]"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &output,
        r####"//*[@class="title"]/a[@href="#DNE"][text()="[DNE]"]"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &output,
        r####"//*[@class="title"]/a[@href="#conclusion"][text()="Conclusion"]"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &output,
        r####"//h2[@id="s1"]/a[@href="#DNE"][text()="[DNE]"]"####,
        1,
    );
    assert_xpath(&output, r####"//h2/a[@href="#s1"][text()="[DNE]"]"####, 1);
}
...
"###
    ...
    let input = "[#a]\n== A <<b>>\n\n[#b]\n== B <<a>>\n";
    let output = convert(input);
    assert_includes(&output, r####"<h2 id="a">A <a href="#b">B [a]</a></h2>"####);
    assert_includes(&output, r####"<h2 id="b">B <a href="#a">[a]</a></h2>"####);
}
...
"###
    ...
    assert_includes(
        &output,
        r####"<h2 id="a">See <a href="#b">Consult Google</a></h2>"####,
    );
}
...
"###
    ...
    let input = "see <<Section Title>>\n\n[#s1]\n== Section Title\n\n[#s2]\n== Section Title\n";
    let output = convert(input);
    assert_xpath(&output, r####"//a[@href="#s1"]"####, 1);
    assert_xpath(
        &output,
        r####"//a[@href="#s1"][text()="Section Title"]"####,
        1,
    );
}
...
"###
    ...
    assert_xpath(
        &output,
        "//a[@href=\"#_cub_tiger\"][text()=\"Cub \u{21D2} Tiger\"]",
        1,
    );
}
