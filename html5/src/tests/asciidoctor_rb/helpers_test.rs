//! Tracks Asciidoctor's `helpers_test.rb`.
//!
//! This file unit-tests `Asciidoctor::Helpers` — internal utility methods
//! (URI-component encoding, `rootname` / `extname?` / basename path handling,
//! and related runtime helpers). Like `attribute_list_test.rb`, it exercises a
//! layer *below* this renderer: each case calls a helper directly and asserts
//! on its return value, with no rendered HTML to observe, so there is nothing
//! here to drive through `convert`. Such parser/runtime-internal behavior
//! belongs to the `asciidoc-parser` layer, not the HTML5 renderer (see the
//! crate memory `html5-favor-convert-over-parse`).
//!
