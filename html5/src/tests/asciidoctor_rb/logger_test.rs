//! Tracks Asciidoctor's `logger_test.rb`.
//!
//! This file unit-tests Asciidoctor's logging infrastructure — `LoggerManager`,
//! the `MemoryLogger` / null logger, severity handling, and message
//! formatting. It exercises runtime diagnostics, not rendered output: each case
//! drives the logger directly and asserts on captured log records, with no HTML
//! to observe through `convert`. This is a parser/runtime-internal concern
//! (indeed, this crate has no logger of its own — see the renderer's
