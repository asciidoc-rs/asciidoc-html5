//! Tracks Asciidoctor's `reader_test.rb`.
//!
//! This file tests Asciidoctor's `Reader` / `PreprocessorReader` — the
//! line-reading and preprocessing layer *beneath* the parser: cursor and line
//! bookkeeping, `include::` resolution, preprocessor conditionals
//! (`ifdef`/`ifndef`/`ifeval`), and source normalization. Each case drives the
//! reader API directly and asserts on the line stream it produces, not on
//! rendered HTML, so there is nothing here to observe through `convert`. This
//! is a parser/runtime-internal layer, which belongs to `asciidoc-parser`, not
//! to the HTML5 renderer (see the crate memory
