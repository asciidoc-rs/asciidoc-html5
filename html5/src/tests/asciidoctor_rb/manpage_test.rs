//! Tracks Asciidoctor's `manpage_test.rb`.
//!
//! This file tests Asciidoctor's **manpage backend** (`convert … to_file …
//! backend: 'manpage'`, asserting on generated `*.roff`/troff output). That
//! backend is explicitly **out of scope** for this project: the README states
//! "DocBook and man page output are not planned" — this renderer emits HTML5
//! only. There is therefore no behavior here to verify against these tests.
//!
//! Unlike the parser-internal files tracked alongside it, this is out of scope
//! by *project decision*, not because it tests a lower layer — the same basis
//! on which the DocBook and XHTML backends are excluded (see the crate memory
//! `xhtml-not-supported`). Tracked entirely as `non_normative!` so the page is
//! honestly accounted for rather than counted as uncovered.
