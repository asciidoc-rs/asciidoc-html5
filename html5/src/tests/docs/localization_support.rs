...
// This crate's own "Localization Support" page. The prose is descriptive
// documentation, tracked as non-normative; the shown conversions are verified
// by the tests below, which drive the `asciidoc_html5` API. The `adoc` crate
// tracks the same page and verifies the CLI-facing examples, and the sdd tool
// merges the two crates' coverage. Only the Rust API example is html5-only;
// every other shown output is verified from both crates.
...
// UTF-8 content is passed through to the output verbatim.
#[test]
...
"#
    ...
    let html = convert("Café, naïve, Über. 日本語. Ελληνικά. مرحبا.");
...
// The `lang` attribute is recorded on the root element of a standalone
// document.
#[test]
...
"#
    ...
    let html = convert_with(
        "= Doc\n\nText.\n",
        &Options::new().standalone(true).attribute("lang", "es"),
    );
...
// `version-label` retitles the revision label in the header; it defaults to the
// English-derived "version" and is rendered downcased.
#[test]
...
"#
    ...
    let default_html = convert_with(source, &Options::new().standalone(true));
    assert!(default_html.contains("<span id=\"revnumber\">version 2.5,</span>"));

    let translated = convert_with(
        source,
        &Options::new()
            .standalone(true)
...
// The Rust API example, verified only from this crate (the `adoc` crate tracks
// the same lines as non-normative).
#[test]
fn version_label_via_the_api() {
    verifies!(
        r#"
...
"#
    ...
    assert!(html.contains("<span id=\"revnumber\">ausgabe 2.5,</span>"));
}
