//! Coverage of the AsciiDoc language description's *Subscript and Superscript*
//! page.
//!
//! A pair of tildes renders subscript (`<sub>`) and a pair of carets renders
//! superscript (`<sup>`), with the restriction that the enclosed run be
//! continuous; a space is written with the `{sp}` attribute reference. This
//! crate produces exactly that, so both examples are verified through
//! `convert`. The title, introduction, section heading, and the closing stem
//! cross-reference are tracked as non-normative.
...
// A pair of tildes makes a continuous run subscript (`<sub>`); a pair of carets
// makes it superscript (`<sup>`).
#[test]
fn subscript_and_superscript_syntax() {
    verifies!(
        r#"
...
"#
    ...
    assert!(out.contains("H<sub>2</sub>O"));
    assert!(out.contains("E=mc<sup>2</sup>"));
    assert_css(&out, "sub", 1);
    assert_css(&out, "sup", 1);
}

// A space inside superscript or subscript text is written with the `{sp}`
// attribute reference, since the run may not contain a literal space.
#[test]
fn superscript_containing_spaces() {
    verifies!(
        r#"
...
"#
    ...
    let out = convert("The deepest body of water is Deep Creek Lake.^[citation{sp}needed]^\n");
    assert!(out.contains("Deep Creek Lake.<sup>[citation needed]</sup>"));
}
