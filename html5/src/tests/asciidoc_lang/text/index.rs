//! Coverage of the AsciiDoc language description's *Text Formatting and
//! Punctuation* index page.
//!
//! This overview page defines formatting marks and pairs and the
//! constrained/unconstrained distinction, then surveys the individual inline
//! styles (each of which has its own page). The conceptual definitions and the
//! per-style survey are descriptive, so they are tracked as non-normative; the
//! constrained and unconstrained *strong* examples render through `convert` and
//! are verified.
...
// A constrained pair uses a single opening and closing mark around a word or
// phrase bounded by space-like characters or punctuation. Each example renders
// the `*strong*` as a `<strong>` element.
#[test]
...
"#
    ...
    let word = convert("That is *strong* stuff!\n");
    assert!(word.contains("That is <strong>strong</strong> stuff!"));

    let phrase = convert("That is *really strong* stuff!\n");
    assert!(phrase.contains("That is <strong>really strong</strong> stuff!"));

    let punctuation = convert("This stuff is *strong*!\n");
    assert!(punctuation.contains("This stuff is <strong>strong</strong>!"));
}
...
// An unconstrained pair uses double marks and can format bounded characters
// within a word, where a constrained pair would not apply.
#[test]
...
"#
    ...
    let out =
        convert("The man page, short for **man**ual page, is a form of software documentation.\n");
    assert!(out.contains("short for <strong>man</strong>ual page"));
}
