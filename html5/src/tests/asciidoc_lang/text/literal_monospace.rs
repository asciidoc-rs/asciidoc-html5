//! Coverage of the AsciiDoc language description's *Literal Monospace* page.
//!
//! Monospace text is not literal in AsciiDoc: it is subject to the normal
//! substitutions. Enclosing the monospaced text in a `+…+` passthrough (inside
//! the backticks) suppresses those substitutions, and the more formal `pass:[]`
//! macro handles content that itself contains plus characters. This crate
//! reproduces both forms, so the two examples are verified through `convert`.
//! The title, the explanatory introduction, the passthrough cross-reference,
//! and the large commented-out legacy-syntax appendix are tracked as
//! non-normative.
...
// Wrapping the monospaced text in a `+…+` passthrough makes it literal: the
// attribute reference `{name}` is emitted verbatim inside the `<code>` element
// rather than being expanded.
#[test]
fn literal_monospace() {
    verifies!(
        r#"
...
// When the literal text itself contains plus characters, the formal `pass:[]`
// macro inside the backticks produces the literal monospace span.
#[test]
fn literal_monospace_with_plus() {
    verifies!(
        r#"
...
"#
    ...
    let out = convert("`pass:[++]` is the increment operator in C.\n");
    assert!(out.contains("<code>++</code> is the increment operator in C."));
}
