//! Coverage of the AsciiDoc language description's *Links* page.
//!
//! This is the overview page for AsciiDoc's linking mechanisms (autolinks, the
//! URL/link/mailto macros — each documented in depth on its own page). The two
//! concrete behaviors it specifies are verified here through `convert`: a URL
//! with URI-encoded reserved characters is autolinked verbatim, and the
//! `hide-uri-scheme` attribute drops the scheme from the displayed link text
//! (while keeping it in the `href`), matching Asciidoctor 2.0.26. The overview
//! prose and the page's trailing `////` comment block are non-normative.
...
// A URL whose reserved characters are URI-encoded (`%22`, `%20`, …) is
// autolinked verbatim — the encoding keeps the URL from being mangled.
#[test]
...
"#
    ...
    // The encoded URL autolinks with its percent-encoded characters intact.
    let output = convert("https://example.org?q=label:%22Requires%20docs%22");
    assert!(output.contains(
        r#"<a href="https://example.org?q=label:%22Requires%20docs%22" class="bare">https://example.org?q=label:%22Requires%20docs%22</a>"#
    ));
}

// With `hide-uri-scheme` set, a bare-URL link drops the scheme from its display
// text but keeps it in the `href`.
#[test]
fn hide_the_url_scheme() {
    verifies!(
        r#"
[#hide-uri-scheme]
== Hide the URL scheme
...
"#
    ...
    // The scheme (`https://`) is hidden in the text but kept in the `href`.
    let output = convert(":hide-uri-scheme:\n\nhttps://asciidoctor.org");
    assert!(
        output.contains(r#"<a href="https://asciidoctor.org" class="bare">asciidoctor.org</a>"#)
    );
}

// The trailing `////` block is commented-out legacy content (an earlier draft
// of the autolinks/URL-macro/link-macro material now documented on the
// dedicated pages), so it is not rendered.
non_normative!(
    r#"
////
= URLs and Links
