//! Coverage of the AsciiDoc language description's *Adjust Image Sizes* page.
//!
//! The `width` and `height` attributes (the second and third positional
//! attributes) map to the same-named attributes on the HTML `<img>` element;
//! that mapping is verified through `convert`. The PDF/DocBook sizing
//! attributes (`pdfwidth`, `scaledwidth`, `scale`), the responsive-scaling
//! stylesheet behavior, and the backend-conditional attribute-cascade example
//! do not affect this crate's HTML output and are non-normative.
...
// The `width`/`height` attributes (or the second/third positional attributes)
// map to the `<img>` element's `width`/`height`.
#[test]
fn width_and_height() {
    verifies!(
        r#"
...
"#
    ...
    assert!(convert("image::flower.jpg[Flower,640,480]")
        .contains(r#"<img src="flower.jpg" alt="Flower" width="640" height="480">"#));

    // The equivalent long-hand named form.
    assert!(
        convert("image::flower.jpg[alt=Flower,width=640,height=480]")
            .contains(r#"<img src="flower.jpg" alt="Flower" width="640" height="480">"#)
    );
}

// The responsive-scaling behavior is a stylesheet feature, not something the
// static `<img>` markup expresses.
non_normative!(
    r#"
...
// `pdfwidth` and `scaledwidth`/`scale` size images for Asciidoctor PDF and the
// DocBook toolchain; they have no effect on this crate's HTML output.
non_normative!(
    r#"
...
// The sizing recap table and the backend-conditional attribute-cascade example
// span DocBook and PDF backends; the html row (`width=`/`role=`) is exercised
// on the other image pages, and the cascade technique is documentation
// guidance.
non_normative!(
    r#"
== Image sizing recap
