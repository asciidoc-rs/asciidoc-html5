//! Coverage of the AsciiDoc language description's *Document to Document Cross
//! References* page.
//!
//! An inter-document `xref:target.adoc#id[text]` macro links to another
//! AsciiDoc document, resolving the target's `.adoc` extension to the output
//! extension (`.html`) — unless `relfilesuffix` overrides it (for a
//! source-to-source reference) or `relfileprefix` prepends to the resolved
//! path. All verified through `convert`, matching Asciidoctor 2.0.26.
//!
//! Some examples are pulled in with `include::example$xref.adoc[tag=…]`; the
...
// An inter-document xref (`xref:document-b.adoc#section-b[…]`) resolves the
// target's `.adoc` extension to the output extension (`.html`).
#[test]
fn inter_document_reference() {
    verifies!(
        r#"
...
"#
    ...
    // The `tag=base-inter` snippet.
    let output =
        convert("Refer to xref:document-b.adoc#section-b[Section B] for more information.");
    assert!(output.contains(r##"<a href="document-b.html#section-b">Section B</a>"##));
}

// The included-in-same-document outcome (the document part dropped) needs an
// include scenario this inline test does not set up.
non_normative!(
    r##"
...
// Setting `relfilesuffix` overrides the output extension for inter-document
// references, enabling a source-to-source (`.adoc`) reference.
#[test]
fn relfilesuffix() {
    verifies!(
        r#"
...
// Setting `relfileprefix` (with `relfilesuffix`) prepends to and rewrites the
// resolved inter-document path.
#[test]
fn relfileprefix() {
    verifies!(
        r#"
...
"#
    ...
    // With `relfileprefix: ../` and `relfilesuffix: /`, an inter-document xref
    // *macro* resolves `filename.adoc` to `../filename/`.
    assert!(convert(
        ":relfileprefix: ../\n:relfilesuffix: /\n\nSee xref:filename.adoc[link text]."
    )
    .contains(r#"<a href="../filename/">link text</a>"#));
}

// The page states the `<<filename.adoc,link text>>` *shorthand* resolves the
// same way, but Asciidoctor 2.0.26 (and this crate) treat a dotted shorthand
// target as an intra-document ID (`href="#filename.adoc"`); only the `xref:`
// macro form resolves inter-document, so the shorthand claim is non-normative.
non_normative!(
    r#"
