//! Coverage of the AsciiDoc language description's *Icons* page.
//!
//! This is the overview page for the icons feature: it introduces the three
//! icon modes (text, image, font), the `icons` document attribute that selects
//! them, and where icons appear (admonition labels, the icon macro, callout
//! numbers). It is entirely descriptive — the concrete rendering behavior of
//! each mode is verified on the mode-specific pages (*Font Icons Mode*, *Image
//! Icons Mode*, and *Icon Macro*) — so the whole page is tracked as
//! non-normative.
