//! Coverage of the AsciiDoc language description's *Blocks* index page.
//!
//! This page is a conceptual overview of blocks — their forms, content models,
//! context, block style, and commonalities, including the reference table of
//! built-in contexts — all tracked as non-normative. The concrete syntax
//! examples it shows are verified through `convert`: a section, a source block,
//! a listing masquerade, and a sidebar carrying a title and ID.
...
// A section block: a title prefixed with equal signs plus its following content
// renders as a `sect1` wrapper with an `<h2>` title and a `sectionbody`.
#[test]
fn section_block_renders_a_section_wrapper() {
    verifies!(
        r#"
----
== Section Title
...
"#
    ...
    assert_css(&output, "div.sect1 > h2", 1);
    assert_css(&output, "div.sect1 > div.sectionbody", 1);
}
...
// A source block: the `source` style specializes the `listing` context; with a
// language it renders the highlighted `<pre class="highlight"><code>` shape.
#[test]
fn source_style_specializes_the_listing_context() {
    verifies!(
        r#"
[source]
....
...
// A listing masquerade: the `[listing]` style on a `....` literal container
// changes the context to `listing`, so it renders as a `listingblock`.
#[test]
fn listing_style_masquerades_a_literal_as_a_listing() {
    verifies!(
        r#"
[source]
----
...
// A sidebar carrying both a title and an ID: the metadata lines stack above the
// opening delimiter, giving the `sidebarblock` its `id` and a title inside the
// content div.
#[test]
fn stacked_metadata_gives_a_sidebar_its_id_and_title() {
    verifies!(
        r#"
----
.Styles of music
...
"#
    ...
    assert_css(&output, "div#music-styles.sidebarblock", 1);
    assert_xpath(
        &output,
        r#"//div[@id="music-styles"]/div[@class="content"]/div[@class="title"][text()="Styles of music"]"#,
        1,
    );
}
