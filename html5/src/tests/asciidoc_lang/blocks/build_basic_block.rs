//! Coverage of the AsciiDoc language description's *Build a Basic Block* page.
//!
//! The page is largely a step-by-step tutorial whose listings show the
//! progressive (and intentionally incomplete) construction of a delimited
//! sidebar; those instructional listings and the closing summary table carry no
//! single verifiable rendering, so they are non-normative. The one section with
//! concrete examples — building a block from a styled paragraph — is verified
//! through `convert`: a `[listing]` paragraph becomes a `listingblock` and a
//! `[quote]` paragraph becomes a `quoteblock`.
//!
//! The single-line examples come from
//! `ref/asciidoc-lang/docs/modules/blocks/examples/block.adoc`.
...
// Building a block from a styled paragraph: on contiguous content, the block
// style name in an attribute list promotes the paragraph to that block type — a
// `[listing]` paragraph becomes a `listingblock`, and a `[quote]` paragraph
// becomes a `quoteblock`.
#[test]
fn styled_paragraph_is_promoted_to_the_named_block() {
    verifies!(
        r#"
...
"#
    ...
    assert_css(&quote, "div.quoteblock > blockquote", 1);
}

// The summary table of built-in block styles is a reference table included from
// a partial; it carries no rule to verify here.
non_normative!(
    r#"
