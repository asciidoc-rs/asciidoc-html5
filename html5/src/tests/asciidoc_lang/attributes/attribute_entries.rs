//! Coverage of the AsciiDoc language description's *Attribute Entries* page.
//!
//! This page introduces the attribute entry syntax. Most of it describes where
//! entries may be declared and how they behave — parser concerns with no
//! distinct rendering — so it is tracked largely as non-normative. The one
//! renderer-observable claim, that an attribute reference in an entry's value
//! is resolved immediately (so a later reference sees the composed value), is
...
#[test]
fn attribute_reference_in_value_resolved_immediately() {
    verifies!(
        r#"
...
"#
    ...
    crate::tests::assert_html::assert_css(
        &output,
        r#"a[href="https://example.org/projects/project-name"]"#,
        1,
    );
}
