...
//! confirming the Table of Contents is rendered. The override examples
//! (`doctype`, the asset-directory attributes) are shown only as source input
//! with no rendered result on the page, so those sections are tracked as
//! non-normative.
...
// The override examples are shown only as source input on the page, with no
// rendered result to check; the `doctype=book` distinction and the
// asset-directory attributes (`imagesdir`, etc.) only manifest in constructs
// this section does not exercise. Their parsing is covered by
// `asciidoc-parser`.
non_normative!(
    r#"
