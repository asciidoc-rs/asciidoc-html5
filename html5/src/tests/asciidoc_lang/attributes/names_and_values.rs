//! Coverage of the AsciiDoc language description's *Attribute Entry Names and
//! Values* page.
//!
//! This page specifies the grammar for attribute names and the kinds of values
//! attributes accept — parser concerns with no distinct rendering — so it is
//! tracked largely as non-normative. The one renderer-observable claim, that an
//! attribute name is lower-cased before it is stored (so `URL-REPO` is
//! referenced as `{url-repo}`), is verified through `convert`.
...
"#
    ...
    // The attribute is set as `URL-REPO` but stored as `url-repo`, so the
    // lowercase `{url-repo}` reference resolves to its value.
    let output = crate::convert(":URL-REPO: https://example.com\n\n{url-repo}");
    crate::tests::assert_html::assert_css(&output, r#"a[href="https://example.com"]"#, 1);
}
