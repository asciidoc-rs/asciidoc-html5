//! Coverage of the AsciiDoc language description's *Description Lists With
//! Marker* page.
//!
//! This page documents an experimental hybrid list — an "unordered" or
//! "ordered" description list — in which the `unordered`/`ordered` block style
//! turns a description list into a marked list whose subject is bold and offset
//! by a stop character. As the page itself states, only Asciidoctor PDF
//! implements this syntax; the HTML backend (this crate's parity oracle,
//! Asciidoctor 2.0.26) does not, and neither does this crate. `[unordered]` on
//! a description list simply passes the style through as a class on a plain
//! `<div class="dlist unordered"><dl>` — the marked-list rendering the page
//! describes is not produced.
//!
//! Because the entire page describes rendering behavior neither Asciidoctor's
//! HTML backend nor this crate delivers, every line is tracked as
//! non-normative. When the AsciiDoc language standardizes this feature and the
//! HTML backend implements it, this page can be revisited and its claims
//! verified.
