//! Coverage of the AsciiDoc language description's *Key Concepts* page.
//!
//! The page is a glossary-style orientation to the vocabulary of AsciiDoc —
//! document, elements, attributes, macros, and preprocessor directives — meant
//! to familiarize a reader with the terms before the rest of the documentation
//! covers each in depth. It defines concepts rather than specifying renderer
//! behavior (the two `[source]` blocks it shows are illustrative macro
//! *syntax*, not rendered output to compare against), so the whole page is
//! non-normative here.
...
// The page is entirely conceptual: it names and defines the building blocks of
// an AsciiDoc document so the reader can recognize the terminology later. No
// section states a rule for the renderer to verify.
non_normative!(
    r#"
= Key Concepts
