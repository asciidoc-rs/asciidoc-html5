//! Coverage of the AsciiDoc language description's *Frequently Asked Questions*
//! page.
//!
//! The FAQ is explanatory prose about the language and its ecosystem — the
//! meaning of "Ascii" in the name, the converter/backend relationship, the
//! (unregistered) media type, and troubleshooting advice for headers and
//! delimited blocks. None of that is a rendering contract, so it is tracked as
//! non-normative. The one concrete, this-crate claim is that AsciiDoc supports
//! the full range of Unicode, not just US-ASCII; that is verified through
//! `convert`.
...
// The document title, the page alias, and the first question's heading.
non_normative!(
    r#"
...
// AsciiDoc content may contain characters from any Unicode range, not just
// US-ASCII. This crate processes a UTF-8 string and carries non-ASCII content
// through to the output unchanged — including the multibyte punctuation, CJK
// text, and emoji below. (The UTF-16-with-a-BOM path is a decoding concern for
// whatever reads the source into a `&str`; `convert` is handed UTF-8 already,
// so it isn't exercised here.)
#[test]
fn asciidoc_supports_full_unicode() {
    verifies!(
        r#"
No.
...
"#
    ...
    let output = convert("Étoilé café — 日本語のテキスト 👌\n");
    assert!(output.contains("Étoilé café — 日本語のテキスト 👌"));
}

// The remainder of the FAQ: what "Ascii" refers to in the name, the
// converter/backend relationship, the proposed (still unregistered) media type,
// and troubleshooting advice for ignored attributes, unclosed delimited blocks,
// and URLs containing substitution characters. All explanatory, none a
// rendering rule this crate owns.
non_normative!(
    r#"
