//! Coverage of the AsciiDoc language description's *Compare AsciiDoc to
//! Markdown* page.
//!
//! The page is a persuasive, side-by-side comparison: prose arguing that
//! AsciiDoc scales better than Markdown, followed by a large table pairing each
//! Markdown construct with its AsciiDoc equivalent. The table cells show source
//! *syntax* literally (inside `[source]` listings), not rendered output to
//! compare against, and the page's claims are comparative ("more concise",
//! "handles cases Markdown doesn't") rather than statements of renderer
//! behavior. So the whole page is tracked as non-normative here; the actual
//! rendering of each construct shown is verified on the language page that
//! documents that construct.

use crate::tests::sdd::*;

track_file!("ref/asciidoc-lang/docs/modules/ROOT/pages/asciidoc-vs-markdown.adoc");

// The full page: the motivation ("write without friction"), the "Starting with
// Markdown" and "Graduating to AsciiDoc" arguments, the comparison table, and
// the trailing commented-out section. It compares the two languages' syntax; it
// does not specify output this renderer must produce.
non_normative!(
    r#"
