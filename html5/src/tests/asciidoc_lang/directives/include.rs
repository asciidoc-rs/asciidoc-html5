//! Coverage of the AsciiDoc language description's *Includes* page — the main
//! reference for the `include` directive.
//!
//! The behaviors verified through `convert` are: an include merges the target's
//! content into the document (the simplest case); under the `Secure` safe mode
//! the directive is converted to a link instead of being read; a
//! backslash-escaped directive is emitted literally; a missing target inserts
//! an "Unresolved directive" message and raises a warning, while
//! `opts=optional` drops it silently; an included AsciiDoc file is itself
//! preprocessed (so includes nest); and a non-AsciiDoc file is merged as-is
//! with no directive processing. The syntax reference, the resolution rules,
//! the encoding/BOM normalization, and the empty-line advice are non-normative.

use asciidoc_parser::warnings::WarningType;

use super::{convert_including, convert_secure, include_warnings};
...
// The include directive imports the target file's content into the document; in
// its simplest form the target is the only argument. Here the fixture
// `partial.adoc` is merged in and rendered as a paragraph.
#[test]
fn an_include_merges_the_targets_content() {
    verifies!(
        r#"
[#include-syntax]
...
"#
    ...
    let output = convert_including("include::partial.adoc[]\n");
    assert!(
        output.contains("The included content lands here."),
        "{output}"
    );
}
...
// Under the `Secure` safe mode the include directive is disabled: rather than
// reading the target, the processor converts the directive to a link to it.
#[test]
fn secure_mode_converts_an_include_into_a_link() {
    verifies!(
        r#"
...
"#
    ...
    let output = convert_secure("include::partial.adoc[]\n");
    assert_css(&output, r#"a[href="partial.adoc"]"#, 1);
    assert!(
        !output.contains("The included content lands here."),
        "{output}"
    );
...
// A backslash-escaped include directive is not processed; it is emitted
// literally, even inside a verbatim block (the directive has no awareness of
// the surrounding structure).
#[test]
fn an_escaped_include_directive_is_not_processed() {
    verifies!(
        r#"
...
"#
    ...
    let escaped = convert_including("\\include::partial.adoc[]\n");
    assert!(escaped.contains("include::partial.adoc[]"), "{escaped}");
    assert!(
        !escaped.contains("The included content lands here."),
        "{escaped}"
    );

    // The escape is required even inside a verbatim block.
    let in_block = convert_including("----\n\\include::partial.adoc[]\n----\n");
    assert!(in_block.contains("include::partial.adoc[]"), "{in_block}");
    assert!(
        !in_block.contains("The included content lands here."),
        "{in_block}"
    );
}
...
// A missing include target does not abort the conversion: the directive is
// replaced with an "Unresolved directive" message in the output and an
// include-file-not-found warning is raised. Adding `opts=optional` instead
// drops the missing include silently, with no message.
#[test]
fn a_missing_target_inserts_a_message_unless_marked_optional() {
    verifies!(
        r#"
...
"#
    ...
    let required = "include::no-such-file.adoc[]\n\ntrailing content";
    let output = convert_including(required);
    assert!(output.contains("Unresolved directive"), "{output}");
    assert!(output.contains("include::no-such-file.adoc[]"), "{output}");
    assert!(output.contains("trailing content"), "{output}");

    let warnings = include_warnings(required);
    assert!(
        warnings
            .iter()
    ...
    assert!(optional.contains("trailing content"), "{optional}");

    // ... and, as the page states, without emitting the not-found warning.
    let optional_warnings = include_warnings(optional_src);
    assert!(
        !optional_warnings
            .iter()
            .any(|(w, _)| matches!(w, WarningType::IncludeFileNotFound(_))),
        "opts=optional should suppress the not-found warning, got {optional_warnings:?}"
    );
}
...
// When the target is an AsciiDoc file, the processor runs its preprocessor over
// the included lines — so an include directive inside the included file is
// itself resolved, letting includes nest.
#[test]
fn an_included_asciidoc_file_is_itself_preprocessed() {
    verifies!(
        r#"
...
"#
    ...
    // `outer.adoc` itself contains `include::partial.adoc[]`; both its own text
    // and the nested include's content appear.
    let output = convert_including("include::outer.adoc[]\n");
    assert!(output.contains("Outer intro."), "{output}");
    assert!(
        output.contains("The included content lands here."),
        "{output}"
    );
...
// A non-AsciiDoc file is merged as-is (after normalization), with no processing
// of AsciiDoc directives — here a CSV file populates a CSV table.
#[test]
fn a_non_asciidoc_file_is_merged_as_is() {
    verifies!(
        r#"
...
"#
    ...
    assert!(output.contains("Sales"), "{output}");
    assert!(output.contains("2016"), "{output}");
}
